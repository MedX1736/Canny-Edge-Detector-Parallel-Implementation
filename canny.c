...
void canny_edge_detect(const uint8_t *input_image, int height, int width,
  ...
  uint8_t *gaussian_blur_output = (uint8_t*) malloc(image_size * sizeof(uint8_t));
  double *gradient_magnitude = (double*) malloc(image_size * sizeof(double));
  uint8_t *gradient_direction = (uint8_t*) malloc(image_size * sizeof(uint8_t));
  double *nms_output = (double*) malloc(image_size * sizeof(double));
  uint8_t *double_thresh_output = (uint8_t*) malloc(image_size * sizeof(uint8_t));
  ...
  free(gaussian_blur_output);
  free(gradient_magnitude);
  free(gradient_direction);
  free(nms_output);
  free(double_thresh_output);
}
    ...
    for (int row = OFFSET; row < height - OFFSET; row++) {
      ...
      if (grad_x_sum == 0.0 || grad_y_sum == 0.0) {
        ...
        magnitude[pixel_index] = ((
            sqrt((grad_x_sum * grad_x_sum) + (grad_y_sum * grad_y_sum))));
        ...
        if ((theta <= 22.5 && theta >= -22.5) || (theta <= -157.5) ||
          ...
          printf("Wrong direction: %d",theta);
      }
