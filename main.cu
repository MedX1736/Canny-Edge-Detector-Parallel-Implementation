...
#include "/content/stb/stb_image_write.h"
...
void canny_edge_detect(const uint8_t *input_image, int height, int width, int high_threshold, int low_threshold, uint8_t *output_image)
{
    ...
    cudaMemcpy(gaussian_blur_input, input_image,
               image_size * sizeof(uint8_t),
               cudaMemcpyHostToDevice);
    /**"Launching gaussian blur kernel..." **/
    gaussian_blur<<<n_blocks, n_threads>>>(gaussian_blur_input, height, width, gaussian_blur_output);
    cudaPeekAtLastError();
  cudaDeviceSynchronize();
  // "Launching gaussian blur kernel finished."
  ...
  gradient_magnitude_direction<<<n_blocks, n_threads>>>(gaussian_blur_output, height, width, gradient_magnitude, gradient_direction);
  cudaPeekAtLastError();
  cudaDeviceSynchronize();
  ...
  cudaMemcpy(nms_output_h, nms_output,image_size * sizeof(double),cudaMemcpyDeviceToHost);
  ...
  uint8_t *double_thresh_output_h = new uint8_t[image_size];
      cudaMemcpy(double_thresh_output_h, double_thresh_output,
                                 image_size * sizeof(uint8_t),
                                 cudaMemcpyDeviceToHost);
      cudaFree(nms_output);
      cudaFree(double_thresh_output);
